...
class MainViewController: UIViewController {
  ...
  private func getTodayNews(){
    ...
    ApiManager.shared.getNews(currentPage: self.currentPage, from: today, to: nil, success: { (newsEntityArray, nextPage) in
...
extension MainViewController: UITableViewDataSource, UITableViewDelegate {
  ...
  func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
    if indexPath.row == listNews.count - 1 && nextPage > currentPage { //here need to check next and current page, and if have next page of that day - should to load. if not, load next day of week
